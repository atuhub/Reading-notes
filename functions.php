...
function reply_to_read($atts, $content=null) {   
        ...
        if ($user_ID > 0) {   
            ...
            //对博主直接显示内容   
            $admin_email = "6210335@qq.com "; //博主Email   
...
function deny_mirrored_websites(){
    $currentDomain = 'wsay." + "me'; //此处自行拆分一下自己的域名即可
...
if($_GET['root'] != 'wenlong')header('Location: http://lomoclub.cc/');
}

/* 替换图片链接为 https */
function http_image_replacer($content){
	if( is_ssl() ){
		/*已经验证使用 $_SERVER['SERVER_NAME']也可以获取到数据，但是貌似$_SERVER['HTTP_HOST']更好一点*/
		$host_name = $_SERVER['HTTPS_HOST'];
...
add_filter('the_content', 'http_image_replacer');
